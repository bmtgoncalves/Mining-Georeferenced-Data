...
#Load Area Average Surge: assume you have queried previously the UBER API and have collected info on pricing for an area
latLongSurge = {}
...
###################
import pylab as plt
surgeValues = [latLongSurge[cy][cx] for cy in range(0, latLen-1) for cx in range(0,lonLen-1) if latLongSurge[cy][cx] !=0]
...
### Leave one out error has been considered as an unbiased estimator of the generalisation error ###
super_predictions = []
super_predictions2 = []
for i in range(0, len(surgeValues)):
